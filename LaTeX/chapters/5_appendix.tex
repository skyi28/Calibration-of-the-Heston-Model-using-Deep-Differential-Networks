...
The correlation parameter ($\rho$) generally adheres to the empirical leverage effect, staying in negative territory between -0.4 and -0.8. However, a regime shift is observable during the 2020-2021 recovery period. During this phase, $\rho$ frequently hits the upper boundary of 0.0, and the volatility of volatility ($\sigma$) simultaneously hits its upper boundary of 1.0. This indicates that during high-stress regimes, the market-implied volatility dynamics exceeded the range covered by the surrogate model, suggesting that a wider sampling domain is necessary for future studies covering extreme market crashes.
...
\begin{figure}[H]
    ...
    \caption{Daily calibrated Heston parameters over the backtesting period (2016-2023).}
    \label{fig:parameters}
\end{figure}

\subsection{Data Source Files for Historical AAPL Options \& Yield Curves}
The historical \ac{aapl} options data utilized in this study is sourced from two comprehensive CSV files obtained from Kaggle. These files encompass daily option quotes spanning from January 2016 to December 2023. The files can be downloaded from the following link: \newline\url{https://www.kaggle.com/datasets/kylegraupe/aapl-options-data-2016-2020}.

The risk-free yield curve data, essential for accurate option pricing, is sourced from the U.S. Department of the Treasury's official website. The daily yield curve rates can be accessed at: \newline\url{https://home.treasury.gov/resource-center/data-chart-center/interest-rates/daily-treasury-rate-archives}.

\subsection{Code Availability}
The code for this study is available on GitHub at \newline\url{https://github.com/skyi28/Calibration-of-the-Heston-Model-using-Deep-Differential-Networks}.